...
for coin, amount in portfolios[user].items():
    ...
    if err or price is None:
        msg += f"{coin} {amount} ≈ ⚠️ price unavailable\n"
        continue
...
try:
    ...
    if r.status_code == 429:
        await update.message.reply_text("⚠️ CoinGecko rate limit — جرب بعد شوية.")
        return

    if r.status_code != 200:
        await update.message.reply_text(f"⚠️ CoinGecko error: {r.status_code}")
        return
    ...
    await update.message.reply_text("⚠️ ماقدرتش أجيب البيانات، جرب بعد شوية.")
```
...
for user_id, alerts in list(price_alerts.items()):
    remaining = []
    for coin, target in alerts:
        symbol = coin + "USDT"
        price, err = await get_binance_price(symbol)
        if err or price is None:
            remaining.append((coin, target))
            continue
        ...
        if current >= target:
            try:
                await context.bot.send_message(
                    ...
                    text=(
                        f"🔔 Price Alert!\n\n"
                        f"{coin} وصل لـ {format_price(current)} USDT\n"
                        f"Target كان: {format_price(target)} USDT"
                    )
                )
            except Exception as e:
...
# Background job: check price alerts every 60 seconds
